      ...
      child: GestureDetector(
        onVerticalDragUpdate: (DragUpdateDetails data) {
          if (_childPosition != null) {
            setState(() {
              _childPosition = Offset(_childPosition!.dx, _childPosition!.dy + data.delta.dy);
              _childDragDelta += data.delta.dy;
            });
          }
        },
        onVerticalDragEnd: (DragEndDetails data){
          final mediaContext = MediaQuery.of(context);
          final visibleScreenSize = mediaContext.size.height - mediaContext.viewInsets.bottom;

          if (_childDragDelta.abs() > visibleScreenSize * 0.1) {
            widget.close();
          } else {
            setState(() {
              _childPosition = Offset(_childPosition!.dx, _childPosition!.dy - _childDragDelta);
              _childDragDelta = 0;
            });
          }
        },
